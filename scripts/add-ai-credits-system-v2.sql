-- Enhanced AI credits system with real OpenAI cost tracking
...
-- Update existing Pro users with initial credits based on their plan type
UPDATE users 
SET 
    ai_credits = CASE 
        WHEN ai_plan_type = 'yearly' THEN 1500  -- €30/year worth of credits
        ELSE 150  -- €3/month worth of credits
    END,
    ai_credits_used = 0,
    ai_monthly_limit = CASE 
        WHEN ai_plan_type = 'yearly' THEN 125  -- 1500/12 monthly limit
        ELSE 150  -- 150 monthly limit
    END,
    ai_credits_reset_date = CASE 
        WHEN ai_plan_type = 'yearly' THEN (NOW() + INTERVAL '1 year')
        ELSE (NOW() + INTERVAL '1 month')
    END
WHERE is_pro = true AND ai_credits = 0;
...
-- Create function to reset monthly credits for Pro users based on plan type
...
BEGIN
    ...
    -- Calculate credits and limits based on plan type
    IF user_record.ai_plan_type = 'yearly' THEN
        new_credits := 1500; -- €30/year ÷ €0.02 = 1,500 credits
        new_limit := 125; -- 1500/12 = 125 per month average
        reset_date := NOW() + INTERVAL '1 year';
    ELSE
        new_credits := 150; -- €3/month ÷ €0.02 = 150 credits
        new_limit := 150;
...
COMMENT ON COLUMN users.ai_credits IS 'Total AI credits available to user';
